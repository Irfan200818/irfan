...
public class FileHelper {
	...
	public static String getFilename(String phoneNumber) throws Exception {
		...
		try {
			...
			file = new File(filepath, Const.FILE_DIRECTORY);
			...
			Log.e(Const.TAG, "Exception " + phoneNumber);
			e.printStackTrace();
		}
	...
	public static void deleteAllRecords(Activity caller) {
		String filepath = getFilePath() + "/" + Const.FILE_DIRECTORY;
	...
	public static List<Model> listDir2(File f, Activity caller) {
		...
		for (File file : files) {
			if (!file.getName().matches(Const.FILE_NAME_PATTERN)) {
				Log.d(Const.TAG, String.format(
	...
	public static List<Model> listFiles(Activity caller) {
		...
		final File file = new File(filepath, Const.FILE_DIRECTORY);
		...
		filepath = caller.getFilesDir().getAbsolutePath();
		final File file2 = new File(filepath, Const.FILE_DIRECTORY);
	...
	public static void deleteFile(String fileName) {
		if (fileName == null)
			return;
		Log.d(Const.TAG, "FileHelper deleteFile " + fileName);
		try {
